...
namespace Drupal\stanford_migrate\Overrides;
 ...
 *
 * @package Drupal\stanford_migrate\Overrides
 */
class MigrationOverrides implements ConfigFactoryOverrideInterface {
  ...
  public function loadOverrides($names) {
    ...
    if (in_array('migrate_plus.migration.stanford_d7_news', $names)) {
      if ($urls = self::getNewsMigrationUrls()) {
        // Point the migration to our local url where we process the feed into
        // usable data.
        $overrides['migrate_plus.migration.stanford_d7_news'] = [
          'status' => TRUE,
          'source' => [
            'urls' => $urls,
          ],
  ...
  protected static function getNewsMigrationUrls() {
    ...
    $field_values = $config_pages->getValue('stanford_migrate_news', 'field_news_xml_feed');
    $urls = [];
